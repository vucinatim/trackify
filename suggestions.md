...
10. Add an explicit, recoverable migration-backup retention command before the next schema migration. The accumulated local snapshots were manually cleared after validation, but Trackify should never silently delete future migration backups; users need a safe way to inspect and prune superseded snapshots once a migrated ledger has soaked successfully. Local app installation now retains at most one rollback bundle and build outputs retain none.
