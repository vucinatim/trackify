...
public struct Bootstrap: ParsableCommand {
    ...
    public struct Inspect: ParsableCommand {
        ...
        public func run() throws {
            ...
            if outputOptions.json {
                ...
                print("Initial reports: at most \(inspection.backfillPlan.maximumProviderCalls) provider calls; inactive days use none")
            }
        }
    }
    ...
    public struct Apply: AsyncParsableCommand {
        ...
        @Option(name: .long, help: "Report mode: auto, codex, claude, or local.")
        var provider = "auto"
        @Option(name: .long, help: "Evidence backfill policy. V1 supports all or none.")
        var backfillEvidence = "all"
        @Option(name: .long, help: "Historical model-report window such as 7d, or none.")
        var backfillReports = "none"
        @Flag(name: .long, help: "Enable model use for future scheduled reports.")
        var enableModelReports = false
        @Flag(name: .long, help: "Confirm the inspected historical model-use plan.")
        var confirmModelBackfill = false
        ...
        public mutating func run() async throws {
            ...
            settings.summaryProvider = settings.providerSelection.explicitProvider
            settings.scheduledModelReportsEnabled = enableModelReports
            ...
            do {
                ...
                generatedReports = try await generateRecentReports(
                    backfillReports,
                    store: store,
                    settings: settings,
                    confirmed: confirmModelBackfill
                )
            ...
            let payload = BootstrapApplyPayload(
                ...
                selectedProvider: settings.summaryProvider,
            ...
            if outputOptions.json {
                ...
                print(
                    "Configured \(try store.discoveryRoots().count) roots and \(settings.providerSelection.rawValue) report mode."
                )
                print("Generated \(generatedReports) recent reports.")
            }
        }
        ...
        private func generateRecentReports(
            _ value: String,
            store: LedgerStore,
            ...
            guard confirmed else {
                throw ValidationError(
                    "Historical model reports require --confirm-model-backfill after inspecting `trackify bootstrap inspect`.")
            }
            let duration = try DateParsing.duration(value)
            guard duration <= 14 * 86_400 else {
                throw ValidationError("Initial report backfill is capped at 14 days to bound provider usage.")
            }
            let now = Date()
            var day = DateParsing.localDay(containing: now.addingTimeInterval(-duration)).start
            var periods: [DateInterval] = []
            while day < now {
                let range = DateParsing.localDay(containing: day)
                periods.append(range)
                day = range.end
            }
            let queue = ReportQueue()
            _ = try queue.backfillPlan(
                store: store, recipeID: RecipeID("daily-work-summary"), periods: periods, cutoff: now)
            _ = try queue.enqueueBackfill(
            ...
            return await queue.drain(
                store: store, settings: settings, maximumRuns: min(20, periods.count)
            ).completed.count
        }
    ...
    public mutating func run() async throws {
        ...
        if preview {
            ...
            if outputOptions.json {
                ...
                print(
                    "Prior hourly reports: \(packet.selection.selectedPriorReportCount); quiet omitted: \(packet.selection.omittedQuietReportCount)"
                )
...
public struct Providers: ParsableCommand {
    public static let configuration = CommandConfiguration(
        abstract: "Inspect and select report providers.",
    ...
    public struct List: ParsableCommand {
        public static let configuration = CommandConfiguration(abstract: "List supported report providers.")
        @OptionGroup var storeOptions: StoreOptions
        @OptionGroup var outputOptions: OutputOptions
        public init() {}
    ...
    public struct Use: ParsableCommand {
        public static let configuration = CommandConfiguration(abstract: "Select the CLI used for generated reports.")
        ...
        public func run() throws {
            ...
            if outputOptions.json {
                ...
                print("Selected \(mode.rawValue) for generated reports.")
            }
        }
    }
...
public struct UsageCommand: ParsableCommand {
    public static let configuration = CommandConfiguration(
        ...
        subcommands: [Today.self, Month.self, Runs.self, Cancel.self])
    public init() {}
...
public struct RecipesCommand: ParsableCommand {
    public static let configuration = CommandConfiguration(
        commandName: "recipes", abstract: "Inspect and create versioned report recipes.",
        subcommands: [List.self, Show.self, Create.self, Preview.self, Backfill.self])
    public init() {}
    ...
    public struct Preview: ParsableCommand {
        ...
        public func run() throws {
            ...
            let packet = try ReportRecipePolicy().apply(
                ReportGenerator().evidencePacket(store: store, range: interval, cutoff: min(now, interval.end)),
                recipe: recipe)
            let payload = ReportPacketPreviewPayload(packet: packet)
            if outputOptions.json {
                try JSONOutput.write(payload)
...
public struct Search: ParsableCommand {
    public static let configuration = CommandConfiguration(
        abstract: "Search repositories, commits, reports, and agent messages."
    )
...
public struct Timeline: ParsableCommand {
    ...
    public func run() throws {
        let now = Date()
        let duration = try DateParsing.duration(since)
        let events = try storeOptions.makeStore().events(
...
public struct Show: ParsableCommand {
    public static let configuration = CommandConfiguration(
        ...
        subcommands: [Session.self, Commit.self, Report.self]
    )
    public init() {}
    ...
    public struct Report: ParsableCommand {
        @Argument var identifier: String
...
public struct DataCommand: ParsableCommand {
    public static let configuration = CommandConfiguration(
        ...
        subcommands: [Path.self, Export.self, DeleteReports.self],
...
public struct Simulate: AsyncParsableCommand {
    ...
    public mutating func run() async throws {
        ...
        if outputOptions.json {
            try JSONOutput.write(SimulationPayload(result: result, dataRoot: directory.path, ephemeral: isEphemeral))
            return
        }
...
private struct ReportPacketPreviewPayload: Encodable {
    ...
    init(packet: ReportEvidencePacket) {
        ...
        priorReportAliases = packet.priorReports.map(\.alias)
    }
}
