...
Installed build `0.3.0-dev` (`513`) recognizes Codex sub-agent activity and
inter-agent communication metadata as non-metric control evidence. Migration 16
reclassified 62 previously unresolved structural records without reading or
changing message text. The live collector now coalesces Git file events to one
scope per working copy and shares generated-directory exclusions with repository
discovery. After restart it converged with zero pending paths, zero pending
triggers, zero consecutive failures, healthy doctor state, and no problems. The
